...
Widget menuDrawer(BuildContext context, MessageController _messageController, Function upDateState) {
  return Drawer(
    child: SafeArea(
      child: Container(
        child: Column(
                ...
                children: [
                  ...
                  if (_messageController.currentUser.isModerator == true)
                    IconButton(
                        ...
                        onPressed: () => pushToCreateChannelView(context, _messageController)),
                ],
              ),
            ),
            Expanded(
...
pushToCreateChannelView(BuildContext context, MessageController _messageController) {
  Navigator.pop(context);
  Navigator.push(context, MaterialPageRoute(builder: (context) => CreateChannelView(_messageController)));
}

Widget itemBuilder(BuildContext context, int num, MessageController _messageController, Function updateState) {
  if (_messageController.currentUser.isModerator == true) {
    return listViewBuilderIfModerator(context, num, _messageController, updateState);
  } else {
    return listViewBuilderIfNotModerator(context, num, _messageController, updateState);
  }
}

Widget listViewBuilderIfModerator(BuildContext context, int num, MessageController _messageController, Function updateState) {
  return drawerListViewComponent(context, num, _messageController, updateState);
}

Widget listViewBuilderIfNotModerator(BuildContext context, int num, MessageController _messageController, Function updateState) {
...
Widget drawerListViewComponent(BuildContext context, int num, MessageController _messageController, Function updateState) {
  return Padding(
